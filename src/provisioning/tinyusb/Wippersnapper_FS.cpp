...
#include <stdlib.h> // malloc/free for the (heap, not stack) boot-region buffers
#endif
...
// once. FAT32 also keeps a backup boot sector at sector 6; this volume is FAT12
// (FM_FAT | FM_SFD) and has none. So we keep our own backup: after a clean
// mount, snapshot the boot region into NVS (a separate flash area that survives
// a format and is unlikely to be hit by the same erase). On a later boot, if
// the region reads back blank (erased), restore it and re-mount, recovering the
// whole filesystem untouched.
//
// "Blank" here is deliberately not "every byte is 0xFF": a real power-loss
// erase is often incomplete and leaves a few stragglers that did not finish
// erasing (we saw exactly one 0x7F byte in a 4 KB region after a hardware
// drain). So we accept the region as erased when it has no valid boot
// signature and only a handful of non-0xFF bytes (see kMaxBlankStragglerDiv).
//
// This is the natural partner to the provisioned-marker guard above: that guard
// stops a power-loss-erased board from being reformatted (so the data is never
// lost); this lets the same board repair itself and boot, instead of only
// halting. It is paranoid - it only ever writes the region back when the live
// region is blank (no boot signature, near-all-0xFF) and the backup passes a
// signature + CRC32 check, so it can only help, never make a bad situation
// worse.
// ---------------------------------------------------------------------------
namespace {
...
#ifdef ARDUINO_ARCH_ESP32
    // Before any halt/format decision, try to repair a boot region that a
    // power-loss glitch erased (boot sector + both FATs) by restoring it from
    // the NVS backup. If it restores, the next initFilesystem() below mounts
    // the recovered volume with every file intact and the gates are skipped.
    restoreBootRegionIfBlank();
#endif
    if (!initFilesystem()) {
...
#ifdef ARDUINO_ARCH_ESP32
  // Filesystem is healthy now (mounted, restored, or freshly formatted):
  // refresh the NVS backup of the boot region so a future power-loss erase of
  // it can be auto-repaired on the next boot. (Not reached if we halted above.)
  backupBootRegion();
#endif
